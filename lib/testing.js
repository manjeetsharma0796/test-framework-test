...
const equalityCriteria = function(datatype) {
  const equalsBy = {
    string: isEqualTo,
    number: isEqualTo,
  ...
  return equalsBy[datatype];
};

const assignDatatype = function(data) {
  let datatype = "";
...
const areEquals = function(actual, expected) {
  if(assignDatatype(actual) !== assignDatatype(expected)) {
    return false;
  }
  const datatype = assignDatatype(actual);
  return equalityCriteria(datatype)(actual, expected);
};
...
exports.equalityCriteria = equalityCriteria;
