...
const isEqualTo = function(actual, expected) {
  return actual === expected;
};

const equalityCheckBy = function(value1, value2) {
  if(datatypeOf(value1) !== datatypeOf(value2)) {
    return false;
  }
  ...
  const datatype = datatypeOf(value1);
  switch(datatype) {
    case("number"): return isEqualTo(value1, value2);
    case("string"): return isEqualTo(value1, value2);
    case("array"): return areArraysEqual(value1, value2);
    case("object"): return areObjectsEqual(value1, value2); 
  }
};

const assertEquals = function(actual, expected, message) {
  const result = isEqualTo(actual, expected);
...
const areArraysEqual = function(array1, array2) {
  ...
  for( let index = 0; index < array1.length; index++) {
    if(!equalityCheckBy(array1[index], array2[index])) {
      return false;
    }
  }
...
exports.equalityCheckBy = equalityCheckBy;
